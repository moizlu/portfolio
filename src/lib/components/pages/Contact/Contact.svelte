...
<section id="contact" class="min-h-dvh h-fit flex flex-col justify-start items-center whitespace-pre">
    <article id="contact-content" class="mt-5 mb-50 w-full section-default flex flex-col justify-start items-center">
        ...
        <form method="POST" action="?/submitContactForm" use:enhance={() => {
            ...
            <p class="text-center text-xs">送信完了後、@moizlu.comのアドレスから<wbr>自動返信メールを送信させていただきます。
                <br>迷惑メールボックスを含めてご確認ください。
                <br>数分経っても届かない場合は再送が可能です。<wbr>(ただし、レート制限がございますので<wbr>ご注意ください。)
                <br>改善しない場合、<wbr>上のボタンから直接メールをお送りください。</p>
