...
<button type="submit" title="send form" disabled={!turnstileState.isVerified} bind:this={submitButtonElement} class="hidden"></button>

<button type="button" {onpointerdown} {onpointerup} onpointerleave={onpointerup} title="send form" disabled={!turnstileState.isVerified} oncontextmenu={(e) => e.preventDefault()} class=" group button-general p-2 enabled:bg-turn-on/30 enabled:hover:bg-turn-on/50 active:bg-turn-on/70">
